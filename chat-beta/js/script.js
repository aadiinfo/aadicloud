...
import { getDatabase, ref, set, get, runTransaction } from "https://www.gstatic.com/firebasejs/11.10.0/firebase-database.js";
...
class PrudenceAIV2 {
    ...
    async updateTokenUsage(tokens) {
        console.log(`Attempting to update token usage with: ${tokens} tokens`);
        const db = getDatabase();
        const tokenRef = ref(db, 'usage/tokensUsed');

        try {
            await runTransaction(tokenRef, (currentTokens) => {
                const newTokens = (currentTokens || 0) + tokens;
                console.log(`Transaction: Current tokens: ${currentTokens}, Tokens to add: ${tokens}, New tokens: ${newTokens}`);
                return newTokens;
            });
            console.log(`Token usage updated successfully by ${tokens} tokens.`);
        } catch (error) {
            console.error("Error updating token usage with transaction:", error);
        }
    }
        ...
        if (cerebrasModelKeys.includes(botId)) {
            try {
                ...
                if (cerebrasResult.success) {
                    ...
                    const tokens = userMessage.length + responseText.length; // Simple token estimation
                    console.log(`Cerebras: Estimated tokens for update: ${tokens}`);
                    console.log(`Cerebras: Calling updateTokenUsage with ${tokens} tokens.`);
        ...
        if (geminiModelKeys.includes(botId)) {
            ...
            try {
                ...
                try {
                    ...
                    const tokens = enhancedMessage.length + geminiResult.text.length; // Simple token estimation
                    console.log(`Gemini: Estimated tokens for update: ${tokens}`);
                    console.log(`Gemini: Calling updateTokenUsage with ${tokens} tokens.`);
                    this.updateTokenUsage(tokens);
                    ...
                    if (firstError.message.includes('Rate limit exceeded') || firstError.message.includes('Quota exceeded')) {
                        ...
                        try {
                            geminiResult = await geminiAPI.generateResponse(enhancedMessage, {                                model: alternativeModel,
