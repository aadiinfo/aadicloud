    ...
    function updateTokenUsage() {
        ...
        onValue(usageRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                const totalTokens = data.totalTokens || 'N/A';
                ...
                document.getElementById('tokens-total').textContent = 'N/A';
            }
        }, (error) => {
            console.error('Error fetching token usage:', error);
            document.getElementById('tokens-used').textContent = 'Error';
            document.getElementById('tokens-total').textContent = 'Error';
        });
    }

    // Initial call to update token usage
    updateTokenUsage();

    // In admin.js, we are not creating users, but we should make sure that the path we are writing to in script.js is consistent
    // with what we are reading here. For now, let's assume that the total tokens are set for all users.
    const db = getDatabase();
    set(ref(db, 'usage/totalTokens'), 50000);
